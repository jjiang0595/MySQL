...
SELECT first_name, IFNULL(AVG(grade), TRIM(0)) FROM students
LEFT JOIN papers ON students.id = papers.student_id GROUP BY first_name ORDER BY AVG(grade) DESC;

SELECT 
    first_name, 
    IFNULL(AVG(grade), TRIM(0)) AS average,
    CASE
